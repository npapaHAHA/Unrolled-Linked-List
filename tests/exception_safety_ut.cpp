...
TEST_F(ExceptionSafetyTest, failsAtInsert) {
    unrolled_list<BadOrGood> unrolled_list;
    ...
    ASSERT_ANY_THROW(unrolled_list.insert(it, Bad{}));


    ASSERT_EQ(unrolled_list.size(), 2);
    ASSERT_EQ(unrolled_list.begin()->Name, "first");
    ASSERT_EQ((++unrolled_list.begin())->Name, "second");
}