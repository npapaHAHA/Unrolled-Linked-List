...
concept SequenceContainer = requires(T a, const T b, typename T::value_type value) {
    ...
    requires requires(typename T::const_iterator pos, const T::const_iterator pos2, typename T::size_type n, typename T::value_type* first, typename T::value_type* last) {
        { a.insert(pos, value) } -> std::same_as<typename T::iterator>;
        { a.insert(pos, n, value) } -> std::same_as<typename T::iterator>;
        { a.insert(pos, first, last) } -> std::same_as<void>;//
