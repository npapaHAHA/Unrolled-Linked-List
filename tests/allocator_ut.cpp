...
TEST_F(WorkWithAllocatorTest, CopyConstructorUsesAllocatorCorrectly) {
    using Alloc = TestAllocator<SomeObj>;
    unrolled_list<SomeObj, 5, TestAllocator<SomeObj>> original;

    for (int i = 0; i < 10; ++i) {
        original.push_back(SomeObj{});
    }

    auto copy = unrolled_list(original, TestAllocator<SomeObj>{});

    ASSERT_EQ(copy.size(), original.size());

    ASSERT_EQ(std::distance(copy.begin(), copy.end()), std::distance(original.begin(), original.end()));

    ASSERT_GT(TestAllocator<NodeTag>::AllocationCount, 0);
    ASSERT_GT(TestAllocator<NodeTag>::ElementsAllocated, 0);
}
